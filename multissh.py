...
class MultiSSHRunner:
    ...
    def add_ssh_job(self, hostname, username=None, password=None, command=None,
        ...
        command -- Option 1: The command to run either as a string or a list of commands
        interaction -- Option 2: An interaction function which will take control
                       of the SSH Client.  The interaction function prototype has to be:
        ...
        if command or interaction:
            self.job_details.append({'hostname': hostname,
                                     'username': username, 'password': password,
                                     'command': command, 'interaction': interaction,
                                     'key_filename': key_filename,
                                     'connect_timeout': connect_timeout})
        else:
            raise Exception('Job command information was missing and therefore the job could not be added')
    ...
    def run(self):
        ...
        def _init_pool():
            # Workaround: Run the atfork function each time we create a connection to avoid the following errors:
            # 'No handlers could be found for logger "paramiko.transport"'
            Crypto.Random.atfork()

            # Workaround: Force ignoring of Ctrl+C so that we can handle it ourselves
            signal.signal(signal.SIGINT, signal.SIG_IGN)

        # Create our multiprocessing workers ensuring we don't waste workers
        pool = multiprocessing.Pool(processes=min(self.processes, len(self.job_details)), initializer=_init_pool)
        try:
            ...
            # Workaround: To ensure proper handling of KeyboardInterrupt, we must wait for each job to complete
            # with a timeout specified.  I have set the timeout to the maximum value possible as we really want
            # an infinite timeout.
...
def _run_job(job_detail):
    """Here is the function run in each thread which uses paramiko to run the appropriate command or interaction.
    This must be outside the class as multiprocessing can't handle a function within a class for its threads.

    Arguments:
    job_detail -- A dict with all the information relating to the job, the dict is formatted as follows:
                  {'hostname': hostname, 'username': username, 'password': password,
                   'command': command, 'interaction': interaction, 'key_filename': key_filename,
                   'connect_timeout': connect_timeout}
    ...
    try:
        ...
        # Connect to the host
        client.connect(timeout=job_detail['connect_timeout'],
                       hostname=job_detail['hostname'], key_filename=job_detail['key_filename'],
                       username=job_detail['username'], password=job_detail['password'])

        # Run a single command
        if job_detail['command'] and type(job_detail['command']) == types.StringType:
